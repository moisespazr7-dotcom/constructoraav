...
@section('content')
<div class="card">
    <div class="section-title">Contactos / Cotizaciones</div>
    <form class="filters" method="GET" action="{{ route('admin.contacts.index') }}">
        ...
        <div style="min-width: 240px; flex:1;">
            <input class="input" name="q" value="{{ request('q') }}" placeholder="Buscar por nombre, telefono o correo">
        </div>
    ...
    <table class="table">
        ...
        @forelse ($items as $item)
            <tr>
                ...
                <td>
                    <div>{{ $item->project_address }}</div>
                    <small>{{ $item->project_info }}</small>
                </td>
                <td>
                ...
                <td>
                    <form class="actions" method="POST" action="{{ route('admin.contacts.update', $item) }}">
                        @csrf
                        @method('PATCH')
                    ...
                    <form method="POST" action="{{ route('admin.contacts.destroy', $item) }}" style="margin-top:8px;">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <tr>
                <td colspan="4" class="muted">No hay registros.</td>
            </tr>
        @endforelse
        </tbody>
    </table>
